...
struct TemperatureFeedback : public NamedComponent<TemperatureFeedback> {
  ...
  TemperatureFeedback(std::string name, Options& alloptions, Solver*)
    ...
    BoutReal SPnorm =
        Pnorm
        * Omega_ci; // Pressure-source normalisation [Pa/s] or [W/m^3] if converted to energy

    temperature_setpoint = options["temperature_setpoint"]
                               .doc("Desired temperature in eV, upstream (y=0) unless "
                                    "control_target_temperature=true")
                               .as<BoutReal>()
                           / Tnorm;

    control_target_temperature =
        options["control_target_temperature"]
            .doc("Adjust the pressure source to match the upstream (if false) or target "
                 "(if true) temperature")
    ...
    species_list = strsplit(
        options["species_for_temperature_feedback"]
            .doc("Comma-separated list of species to apply the PI-controlled source to")
            .as<std::string>(),
        ',');
    scaling_factors_list =
        strsplit(options["scaling_factors_for_temperature_feedback"]
                     .doc("Comma-separated list of scaling factors to apply to the "
                          "PI-controlled source, 1 for each species")
                     .as<std::string>(),
                 ',');

    if (species_list.size() != scaling_factors_list.size()) {
      throw BoutException("TemperatureFeedback: species_list length doesn't match "
                          "scaling_factors length. Need 1 scaling factor per species.");
    }
    ...
    source_shape = (alloptions[std::string("P") + name]["source_shape"]
                        .doc("Source term in ddt(P" + name
  ...
  void outputVars(Options& state) override {
    if (diagnose) {
      ...
      set_with_attrs(state[std::string("temperature_feedback_src_shape_") + name],
                     source_shape,
                     {{"units", "Pa / s"},
                      {"conversion", SPnorm},
      ...
      set_with_attrs(
          state[std::string("SP") + name + std::string("_feedback")],
          source_shape * source_multiplier,
          {{"time_dimension", "t"},
           {"units", "Pa / s"},
           {"conversion", SPnorm},
      ...
      set_with_attrs(state[std::string("temperature_feedback_src_p_") + name],
                     proportional_term,
                     {{"time_dimension", "t"},
                      {"long_name", name + " proportional feedback term"},
                      {"source", "temperature_feedback"}});

      set_with_attrs(state[std::string("temperature_feedback_src_i_") + name],
                     integral_term,
                     {{"time_dimension", "t"},
  ...
  void restartVars(Options& state) override {
    ...
    if (first and state.isSet(name + "_temperature_error_integral")) {
      ...
      temperature_error_integral =
          state[name + "_temperature_error_integral"].as<BoutReal>();
    }

    // Save the temperature error integral
    set_with_attrs(state[name + "_temperature_error_integral"],
                   temperature_error_integral,
                   {{"long_name", name + " temperature error integral"},
                    {"source", "temperature_feedback"}});
  }

  static constexpr auto type = "temperature_feedback";
...
private:
  ...
  BoutReal temperature_controller_p,
      temperature_controller_i; ///< PI controller parameters
  BoutReal error;
...
namespace {
RegisterComponent<TemperatureFeedback> register_uts;
}
