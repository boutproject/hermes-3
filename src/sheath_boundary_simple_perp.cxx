...
#include "../include/sheath_boundary_simple_perp.hxx"
...
SheathBoundarySimplePerp::SheathBoundarySimplePerp(std::string name, Options& alloptions, Solver*)
  ...
  inner_x = options["inner_x"].doc("Boundary on inner x?").withDefault<bool>(true);
  outer_x = options["outer_x"].doc("Boundary on outer x?").withDefault<bool>(true);
...
void SheathBoundarySimplePerp::transform_impl(GuardedOptions& state) {
  ...
  Field3D electron_sheath_power_xlow = zeroFrom(Ne);
  ...
  if (inner_x && mesh->firstX()) {
   // for (RangeIterator r = mesh->iterateBndryLowerX(); !r.isDone(); r++) {
	  ...
	  -sqrt(tesheath / (TWOPI * Me)) * (1. - Ge) * exp(-(phisheath - phi_wall) / floor(tesheath, 1e-5));
        ...
        electron_sheath_power_xlow[i] += heatflow;       // [W], lower X, so sheath boundary power placed in final domain cell 
  ...
  if (outer_x && mesh->lastX()) {
    ...
    for (int iy = 0; iy < mesh->LocalNy; iy++)  { // TODO: that is not going to work with SOL geometry, need to loop over the actual boundary points
      for (int jz = 0; jz < mesh->LocalNz; jz++) {
        ...
        q = gamma_e * tesheath * nesheath * vesheath;   // [Wm^-2]
        hflux_e[i] -= q * da / dv;   // [Wm^-3]
        electron_sheath_power_xlow[ip] += heatflow;    // [W]  Upper X, so sheath boundary power on xlow side of inner guard cell
  ...
  for (auto& kv : allspecies.getChildren()) {
    ...
    if (inner_x && mesh->firstX()) {
      for (int iy = 0; iy < mesh->LocalNy; iy++) {
        for (int jz = 0; jz < mesh->LocalNz; jz++) {
          ...
          ion_sheath_power_xlow[i] += heatflow;      // [W] lower X, so power placed in final domain cell
        }
      }
    }
    if (outer_x && mesh->lastX()) {
      ...
      for (int iy = 0; iy < mesh->LocalNy; iy++)  { // TODO: that is not going to work with SOL geometry, need to loop over the actual boundary points
        ...
        for (int jz = 0; jz < mesh->LocalNz; jz++) {
          ...
          q = gamma_i * tisheath * nisheath * visheath;   // [Wm^-2]
          hflux_i[i] -= q * da / dv;   // [Wm^-3]
          ion_sheath_power_xlow[ip] += heatflow;  // [W]  Upper X, so sheath boundary power on xlow side of inner guard cell
    ...
    // Add the total sheath power flux to the tracker of x power flows
    add(species["energy_flow_xlow"], fromFieldAligned(ion_sheath_power_xlow));
...
void SheathBoundarySimplePerp::outputVars(Options& state) {
  // Normalisations
  ...
  if (diagnose) {
    ...
    for (auto s1 = std::begin(level1); s1 != std::end(level1); ++s1) {
      ...
      set_with_attrs(state[{"E" + species_name + "_sheath"}], getNonFinal<Field3D>(section["energy_source"]),
                            ...
                            {"source", "sheath_boundary_simple_perp"}});
