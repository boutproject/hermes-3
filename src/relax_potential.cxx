...
void RelaxPotential::finally(const Options& state) {
  ...
  if (exb_advection) {
    ...
    if (exb_advection_simplified) {
      ...
      ddt(Vort) -= FV::Div_a_Grad_perp(
          Field3D(Field2D(0.5 * average_atomic_mass / Bsq)), vEdotGradPi);
      ddt(Vort) -=
  ...
  if (boussinesq) {
    ddt(phi1) =
        lambda_1
        * (FV::Div_a_Grad_perp(Field3D(Field2D(average_atomic_mass / Bsq)), phi)
           - Vort);
    ...
    if (diamagnetic_polarisation) {
      for (const auto& kv : allspecies.getChildren()) {
        ...
        ddt(phi1) += lambda_1 * FV::Div_a_Grad_perp(Field3D(Field2D(A / Bsq)), P);
      }
    }
  } else {
    ...
    for (const auto& kv : allspecies.getChildren()) {
      ...
      if (diamagnetic_polarisation and species.isSet("pressure")) {
        ...
        phi_vort += FV::Div_a_Grad_perp(Field3D(Field2D(Ai / Bsq / Zi)), Pi);
      }
    }
