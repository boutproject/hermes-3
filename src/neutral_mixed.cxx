...
void NeutralMixed::finally(const Options& state) {
  ...
  if (ef_adv_perp_xlow.isAllocated()) {
    ...
    ASSERT3(! ef_adv_perp_ylow.isAllocated());
  }
  ...
  if (evolve_momentum) {
	...
	Div_par_K_Grad_par_mod(  // Parallel viscosity 
      ...
      if (!NVn.isFci()) {                                                                     // Perpendicular advection                                                                                          
	viscosity_source += Div_a_Grad_perp_flows(
  ...
  // TODO(dave) do we need a flag for this?
  BOUT_FOR(i, Pn.getRegion("RGN_NOBNDRY")) {
...
void NeutralMixed::precon([[maybe_unused]] const Options& state, BoutReal gamma) {
  ...
  // TODO(dave) is this needed for fci?
