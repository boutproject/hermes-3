...
Field3D Grad_perp_f_B_SQ(const Field3D& f) {
  Field3D result{0.0};
  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++) {
    for (int j = mesh->ystart; j <= mesh->yend; j++) {
      for (int k = 0; k < nz; k++) {
        ...
        // 2) Calculate square velocities on cell faces

        BoutReal v2U =
            (coord->g11(i, j) * SQ((fpp - fmp) / coord->dx(i, j)) // g^xx (df/dx)^2
             + coord->g33(i, j)
                   * SQ((f(i, j, kp) - f(i, j, k)) / coord->dz(i, j))) // + g^zz (df/dz)^2
            / SQ(coord->Bxy(i, j));
        ...
        BoutReal v2R = (0.5 * (coord->g33(i, j) + coord->g33(i + 1, j))
                        ...
                        + 0.5 * (coord->g11(i, j) + coord->g11(i + 1, j))
                              * SQ((f(i + 1, j, k) - f(i, j, k))
                                   / (0.5 * (coord->dx(i, j) + coord->dx(i + 1, j)))))
                       / SQ(0.5 * (coord->Bxy(i, j) + coord->Bxy(i + 1, j)));

        BoutReal v2L = (0.5 * (coord->g33(i, j) + coord->g33(i - 1, j))
                            * SQ((fmp - fmm) / coord->dz(i, j))
                        + 0.5 * (coord->g11(i, j) + coord->g11(i - 1, j))
                              * SQ((f(i, j, k) - f(i - 1, j, k))
                                   / (0.5 * (coord->dx(i, j) + coord->dx(i - 1, j)))))
                       / SQ(0.5 * (coord->Bxy(i, j) + coord->Bxy(i - 1, j)));

        // Average over cell, weighting with J
        result(i, j, k) = 0.25
                          * ((0.5 * (coord->J(i, j) + coord->J(i + 1, j)) * v2R
                              + 0.5 * (coord->J(i, j) + coord->J(i - 1, j)) * v2L)
                                 / coord->J(i, j)
                             + v2U + v2D);
      }
    }
