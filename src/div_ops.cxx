  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++) {
    for (int j = mesh->ystart; j <= mesh->yend; j++) {
      for (int k = 0; k < nz; k++) {
        ...
        if (vU > 0.0) {
          BoutReal flux = vU * s.R / (coord->J(i, j, k) * coord->dz(i, j, k));
          result(i, j, k) += flux;
          result(i, j, kp) -= flux;
        }
        if (vD < 0.0) {
          BoutReal flux = vD * s.L / (coord->J(i, j, k) * coord->dz(i, j, k));
          result(i, j, k) -= flux;
          result(i, j, km) += flux;
        }
      }
...
/// i.e xlow(i,j, k) is the flow into cell (i,j, k) from the left,
///               and the flow out of cell (i-1,j, k) to the right
///
///           ylow(i,j+1)
///              ^
///           +---|---+
///           |       |
/// xlow(i,j, k) -> (i,j, k) -> xlow(i+1,j, k)
///           |   ^   |
///           +---|---+
///           ylow(i,j, k)
///
...
// Functions added for dipole transport

  Field3D Div_a_Grad_perpB_flows(const Field3D& a, const Field3D& f,
                                       Field3D& flow_xlow, Field3D& flow_ylow) {
    ASSERT2(a.getLocation() == f.getLocation());
    ...
    */
    ...
    for (int i = xs; i <= xe; i++) {
      for (int j = mesh->ystart; j <= mesh->yend; j++) {
        for (int k = 0; k < mesh->LocalNz; k++) {
          ...
          BoutReal fout = 0.5
                          * (a(i, j, k) * coord->Bxy(i, j, k)
                             + a(i + 1, j, k) * coord->Bxy(i + 1, j, k))
                          * (coord->J(i, j, k) * coord->g11(i, j, k)
                             + coord->J(i + 1, j, k) * coord->g11(i + 1, j, k))
                          * (f(i + 1, j, k) / coord->Bxy(i + 1, j, k)
                             - f(i, j, k) / coord->Bxy(i, j, k))
                          / (coord->dx(i, j, k) + coord->dx(i + 1, j, k));
    ...
    // Values on this y slice (centre).
    // This is needed because toFieldAligned may modify the field
    Field3D fc = f * coord->Bxy();
    Field3D ac = a * coord->Bxy();
    ...
    if (f.hasParallelSlices() && a.hasParallelSlices()) {
      ...
      fup = f.yup() * coord->Bxy().yup();
      fdown = f.ydown() * coord->Bxy().ydown();

      aup = a.yup() * coord->Bxy().yup();
      adown = a.ydown() * coord->Bxy().ydown();
    } else {
      // At least one input doesn't have yup/ydown fields.
      // Need to shift to/from field aligned coordinates
    ...
    for (int i = xs; i <= xe; i++){
      for (int j = mesh->ystart; j <= mesh->yend; j++) {
        for (int k = 0; k < mesh->LocalNz; k++) {
          ...
          // Use the upwind coefficient
          const BoutReal fout = (coord->J(i, j)
                                + coord->J(i+1, j))/2.0 * (sqrt(coord->g11(i, j)) + sqrt(coord->g11(i, j))) /2.0
      ...
      for (int i = xs; i <= xe; i++)
        for (int j = mesh->ystart; j <= mesh->yend; j++) {
          for (int k = 0; k < mesh->LocalNz; k++) {
            ...
            const BoutReal fout =
                gradient
                * ((gradient > 0) ? a(i + 1, j, k) * coord->Bxy(i + 1, j, k)
                                  : a(i, j, k) * coord->Bxy(i, j, k));
      ...
      for (int i = mesh->xstart; i <= mesh->xend; i++) {
        for (int j = mesh->ystart; j <= mesh->yend; j++) {
          ...
          for (int k = 0; k < mesh->LocalNz; k++) {
            ...
            BoutReal dfdz =
                0.25
                * (fc(i, j, kp) - fc(i, j, km) + fup(i, j + 1, kp) - fup(i, j + 1, km))
                / coord->dz(i, j);
            ...
            // Calculate flux between j and j-1
            dfdz = 0.25
                   * (fc(i, j, kp) - fc(i, j, km) + fdown(i, j - 1, kp)
                      - fdown(i, j - 1, km))
                   / coord->dz(i, j);
