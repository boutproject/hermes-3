...
EvolveDensity::EvolveDensity(std::string name, Options& alloptions, Solver* solver)
  ...
  density_source_floor =
      options["density_source_floor"]
          .doc("Floor below which an artificial density source is enabled")
          .withDefault(1e-4);

  density_source_strength =
      options["density_source_strength"]
...
void EvolveDensity::finally(const Options& state) {
  ...
  if (density_source_strength > 0.0) {
    ...
    ddt(N) += density_source_strength * (softFloor(N, density_source_floor) - N)
              / density_source_floor;
  }
