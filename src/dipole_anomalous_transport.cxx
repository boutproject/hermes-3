...
DipoleAnomalousDiffusion::DipoleAnomalousDiffusion(std::string name, Options& alloptions,
  ...
  transport_switch =
      options["transport_switch"].doc("transport_switch").withDefault<bool>(false);
  ...
  dipole_quasilinear_v = 0.0;
   dipole_quasilinear_v =
      options["dipole_quasilinear_v"]
          .doc("Dipole quasilinear particle inward velocity [Wb/s]")
          .withDefault(dipole_quasilinear_v) / velocity_norm;
   ...
   BOUT_FOR(i, dipole_quasilinear_D.getRegion("RGN_ALL")) {
     dipole_quasilinear_D[i] = dipole_quasilinear_D[i] / (coord->g11[i]);
    }
    // if (mesh->firstX()) {
  ...
  Coordinates* coord2 = dipole_anomalous_D.getCoordinates();
    ...
    if (include_D) {
      ...
      void DipoleAnomalousDiffusion::transform_impl(GuardedOptions & state) {
        ...
        mesh->communicate(N2D);
        mesh->communicate(B2D);
        Field2D v_B = dipole_quasilinear_D * N2D;
        mesh->communicate(v_B);
        Field2D logB2D = log(B2D);
         dipole_quasilinear_D_p =
            dipole_quasilinear_D * N2D * mu0_normalized * 2 / B2D / B2D;
        ...
        // }
        if (transport_switch)
          transport_on = isnegative_dnBdphi(N2D, B2D, transport_off_factor);
        else 
        {
          transport_on = 1.0;
        }
        ...
        Gamma_N = Div_a_Grad_perp_upwind_flows(dipole_anomalous_D, N2D, flow_xlow, flow_ylow);
      ...
      const Field2D isnegative_dnBdphi(const Field2D& N2D, const Field2D& B2D,
