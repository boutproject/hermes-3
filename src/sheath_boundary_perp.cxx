#include "../include/sheath_boundary_perp.hxx"
...
#if CHECKLEVEL >= 2
  if (!std::isfinite(fp)) {
    throw BoutException("SheathBoundaryPerp limitFree: {}, {} -> {}", fm, fc, fp);
  }
#endif
...
SheathBoundaryPerp::SheathBoundaryPerp(std::string name, Options& alloptions, Solver*)
  ...
  inner_x = options["inner_x"].doc("Boundary on inner x?").withDefault<bool>(false);
  outer_x = options["outer_x"].doc("Boundary on outer x?").withDefault<bool>(false);
...
void SheathBoundaryPerp::transform_impl(GuardedOptions& state) {
  ...
  for (auto& kv : allspecies.getChildren()) {
    ...
    if (inner_x && mesh->firstX()){ // Only do this for the processor which has the edge region
      // for (RangeIterator r = mesh->iterateBndryInnerX(); !r.isDone(); r++) { // That what should be done
        for (int iy = 0; iy < mesh->LocalNy; iy++)  { // TODO: that is not going to work with SOL geometry, need to loop over the actual boundary points
        for (int jz = 0; jz < mesh->LocalNz; jz++) {
          auto i = indexAt(Ne, mesh->xstart, iy, jz);
          auto ip = i.xp();
