...
void EvolvePressure::finally(const Options& state) {
  ...
  if (species.isSet("velocity")) {
    ...
    if (p_div_v) {
      ...
      ddt(P) -= setName(FV::Div_par_mod<hermes::Limiter>(P, V, fastest_wave, flow_ylow_advection), "Div_par_mod(P, V)");
      ...
      ddt(P) -= SETNAME((5. / 3) * FV::Div_par_mod<hermes::Limiter>(P, V, fastest_wave, flow_ylow_advection));

      E_VgradP =  V * Grad_par(P);
      ddt(P) += SETNAME((2. / 3) * E_VgradP);
    }
