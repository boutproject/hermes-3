...
def save_ivertex_indices_to_netcdf(
    source_file, destination_file, Rpoints_full, Zpoints_full
):
    ...
    ivertex_corners = get_boutxx_corner_index(
        data_Rxy, data_Zxy, Rpoints_full, Zpoints_full
    )
    ivertex_corners_lr = get_boutxx_corner_index(
        data_Rxy_lr, data_Zxy_lr, Rpoints_full, Zpoints_full
    )
    ivertex_corners_ul = get_boutxx_corner_index(
        data_Rxy_ul, data_Zxy_ul, Rpoints_full, Zpoints_full
    )
    ivertex_corners_ur = get_boutxx_corner_index(
        data_Rxy_ur, data_Zxy_ur, Rpoints_full, Zpoints_full
    )
    ...
    dataset.setncattr(
        "vertices_calculation_version_git_hash",
            ...
            for ixp in range(0, Nx):
                for iyp in range(0, Ny):
                    if ix == ixp and iy == iyp:
                        continue
                    if isapprox(
                        corners_Rxy[ix, iy], corners_Rxy[ixp, iyp]
                    ) and isapprox(corners_Zxy[ix, iy], corners_Zxy[ixp, iyp]):
                        ic = iy + Ny * ix
...
def unique_points_1D(corners_Rxy, corners_Zxy):
    ...
    for ic in range(0, Nc):
        for icp in range(0, Nc):
            if ic == icp:
                continue
            if isapprox(corners_Rxy[ic], corners_Rxy[icp]) and isapprox(
                corners_Zxy[ic], corners_Zxy[icp]
            ):
                # print("non-unique point:",ix,iy,ic)
                unique = False
    # print("unique points =",unique)
...
def get_cell_vertex_list_inner(
    R_ll, Z_ll, R_lr, Z_lr, R_ur, Z_ur, R_ul, Z_ul, R_vertices, Z_vertices
):
...
def get_cell_vertex_list(
    ...
    for ix in range(0, Nx):
        for iy in range(0, Ny):
            ...
            R_ul = Rxy_ul[ix, iy]
            Z_ul = Zxy_ul[ix, iy]
            cell_vertices[icell, :] = get_cell_vertex_list_inner(
                R_ll, Z_ll, R_lr, Z_lr, R_ur, Z_ur, R_ul, Z_ul, R_vertices, Z_vertices
            )
...
def get_boutxx_corner_index(Rxy_corners, Zxy_corners, R_vertices, Z_vertices):
    ...
    for ix in range(0, Nx):
        for iy in range(0, Ny):
            ...
            iglobal_corners[ix, iy] = convert_R_Z_to_vertex_index(
                Rxy_corners[ix, iy],
                Zxy_corners[ix, iy],
                R_vertices,
                Z_vertices,
...
def get_pfr_lower_boundary_vertices(
    ivertex_corners,
    data_Rxy,
    data_Zxy,
    ivertex_corners_ul,
...
def get_pfr_upper_boundary_vertices(
    ivertex_corners,
    data_Rxy,
    data_Zxy,
    ivertex_corners_ul,
...
def get_vac_left_boundary_vertices(
    ivertex_corners_lr,
    data_Rxy_lr,
    data_Zxy_lr,
    ivertex_corners_ur,
...
def get_vac_right_boundary_vertices(
    ivertex_corners_lr,
    data_Rxy_lr,
    data_Zxy_lr,
    ivertex_corners_ur,
...
def get_core_boundary_vertices(
    ivertex_corners,
    data_Rxy,
    data_Zxy,
    jyseps1_1,
...
def get_target_ll_vertices(
    ivertex_corners, data_Rxy, data_Zxy, ivertex_corners_lr, data_Rxy_lr, data_Zxy_lr
):
    ivertex_target_ll = []
    Rxy_target_ll = []
    Zxy_target_ll = []
...
def plot_corners_get_dmplex_data(
    file_path, interactive_plot=False, print_cells_to_screen_output=False
):
    ...
    if exclude_y_guard_cells:
        s = (
            slice(0, -1, 1),
            list(range(y_boundary_guards, ny_inner + y_boundary_guards))
            + list(
                range(
                    ny_inner + 3 * y_boundary_guards,
                    2 * ny_inner + 3 * y_boundary_guards,
                )
            ),
        )
    else:
    ...
    save_ivertex_indices_to_netcdf(
        file_path, file_path[:-3] + ".corners.nc", Rpoints_full, Zpoints_full
    )
    ...
    cell_vertices, cell_vertices_RZ, vertex_list = get_cell_vertex_list(
        data_Rxy,
        data_Zxy,
        data_Rxy_lr,
        data_Zxy_lr,
    ...
    ivertex_pfr_lower, Rxy_pfr_lower, Zxy_pfr_lower = get_pfr_lower_boundary_vertices(
        ivertex_corners,
        data_Rxy,
        data_Zxy,
        ivertex_corners_ul,
        ...
        y_boundary_guards,
        jyseps1_1,
        exclude_y_guard_cells=exclude_y_guard_cells,
    )

    ivertex_pfr_upper, Rxy_pfr_upper, Zxy_pfr_upper = get_pfr_upper_boundary_vertices(
        ivertex_corners,
        data_Rxy,
        data_Zxy,
        ivertex_corners_ul,
    ...
    ivertex_sol_vac_left, Rxy_sol_vac_left, Zxy_sol_vac_left = (
        get_vac_left_boundary_vertices(
            ivertex_corners_lr,
            data_Rxy_lr,
            data_Zxy_lr,
            ivertex_corners_ur,
    ...
    ivertex_sol_vac_right, Rxy_sol_vac_right, Zxy_sol_vac_right = (
        get_vac_right_boundary_vertices(
            ivertex_corners_lr,
            data_Rxy_lr,
            data_Zxy_lr,
            ivertex_corners_ur,
    ...
    ivertex_core, Rxy_core, Zxy_core = get_core_boundary_vertices(
        ivertex_corners,
        data_Rxy,
        data_Zxy,
        jyseps1_1,
    ...
    ivertex_target_ll, Rxy_target_ll, Zxy_target_ll = get_target_ll_vertices(
        ivertex_corners,
        data_Rxy,
        data_Zxy,
        ivertex_corners_lr,
    ...
    ivertex_target_ul, Rxy_target_ul, Zxy_target_ul = get_target_ul_vertices(
        ivertex_corners_ul,
        data_Rxy_ul,
        data_Zxy_ul,
        ivertex_corners_ur,
    ...
    ivertex_target_ur, Rxy_target_ur, Zxy_target_ur = get_target_ur_vertices(
        ivertex_corners,
        data_Rxy,
        data_Zxy,
        ivertex_corners_lr,
    ...
    ivertex_target_lr, Rxy_target_lr, Zxy_target_lr = get_target_lr_vertices(
        ivertex_corners_ur,
        data_Rxy_ur,
        data_Zxy_ur,
        ivertex_corners_ul,
